...
// Scroll Effects with Performance
let lastScrollY = 0;
let ticking = false;
...
function setupCardTilt() {
  const cards = document.querySelectorAll('.solutionCard, .diffCard, .authority__item, .featuredCard, .projectCard');
...
function setupFadeInAnimations() {
  ...
  document.querySelectorAll('.solutionCard, .diffCard, .authority__item, .featuredCard, .projectCard, .faqItem').forEach(el => {
    observer.observe(el);
  });
}
  ...
  document.querySelectorAll('.btn').forEach(button => {
    button.addEventListener('click', function(e) {
      if (this.href) {
        ...
        const originalText = this.querySelector('span:not(.loading)')?.textContent || this.textContent;
        
        if (loadingSpan) {
          loadingSpan.style.display = 'inline-block';
          if (this.querySelector('span:not(.loading)')) {
...
function renderFaqs() {
  ...
  faqList.innerHTML = activeFaqs.map((faq, index) => `
    <article class="faqItem fade-in" style="animation-delay: ${index * 0.1}s">
...
function renderFeatured() {
  ...
  featuredGrid.innerHTML = list.map((project, index) => {
    ...
    return `
      <article class="featuredCard work fade-in" style="animation-delay: ${index * 0.1}s" data-open-project="${project.id}">
        <div class="work-inner">
          ...
          <div class="featuredCard__body meta">
            <h3 class="title">${escapeHtml(project.title)}</h3>
...
function renderProjects() {
  ...
  projectsGrid.innerHTML = slice.map((project, index) => {
    ...
    const showStatus = shouldShowFrontStatus(project.status);
    return `
      <article class="projectCard work fade-in" style="animation-delay: ${index * 0.1}s" data-open-project="${project.id}">
        <div class="work-inner">
...
// Add animation to WhatsApp buttons
...
window.CIBORG347 = {
  ...
  loadFeaturedProjects: renderFeatured,
  loadAllProjects: renderProjects,
  createParticles,
  setupCardTilt
};
